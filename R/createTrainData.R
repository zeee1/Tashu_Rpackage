...
createTrainData <- function(stationNum){
  ...
  # rentCount : number of rentals in stationNum th station.
  rent_TrainDF <- data.frame(datetime = as.Date(character()), season = character(), rentMonth = character(), rentHour = character(), rentWeekday = character(), temperature = integer(), humidity = integer(), rainfall = integer(), isFestival = character(), rentCount = integer())
